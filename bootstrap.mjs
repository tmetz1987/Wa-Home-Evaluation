// Writes the whole WA-only valuation app during install so you don't need a ZIP.
...
export default nextConfig;`,
  ...
  "tailwind.config.js": `module.exports = { content: ["./app/**/*.{js,jsx}", "./lib/**/*.{js,jsx}"], theme: { extend: {} }, plugins: [] };`,
...
export default function RootLayout({children}){return <html lang="en"><body>{children}</body></html>}`,
  ...
  "lib/waPpsfBaseline.js": `export const WA_ZIP_PPSF={ "98101":720,"98102":680,"98103":620,"98104":580,"98105":650,"98106":500,"98107":610,"98108":480,"98109":700,"98112":760,"98115":640,"98116":660,"98117":620,"98118":520,"98119":740,"98122":630,"98125":540,"98126":560,"98012":430,"98052":600,"98033":640,"98004":950,"98005":820,"98034":560,"98027":650,"98006":780,"98059":520,"98208":380,"98201":360,"98290":360,"98335":360,"98402":340,"99201":300,"99223":280,"98801":280,"99352":270 };`,

  "lib/valuation.js": `import { WA_ZIP_PPSF } from "./waPpsfBaseline";
...
export function extractZip(address){const m=address?.match(/\\b(\\d{5})(?:-\\d{4})?\\b/);return m?m[1]:null}`,
  ...
  "lib/providers.js": `export async function geocodeAddress(address){
...
export async function fetchCompsATTOM(lat,lng,sqft){
  ...
  "app/api/estimate/route.js": `import { NextResponse } from "next/server";
import { z } from "zod";
import { estimateValue } from "@/lib/valuation";
import { geocodeAddress, fetchSubjectFromEstated, fetchSchoolRating, fetchCompsATTOM } from "@/lib/providers";
const InputSchema=z.object({ address:z.string().min(8), sqft:z.number().int().positive().optional(), beds:z.number().int().min(0).max(10).optional(), baths:z.number().int().min(0).max(10).optional(), lotSqft:z.number().int().positive().optional(), yearBuilt:z.number().int().min(1800).max(new Date().getFullYear()).optional(), condition:z.number().int().min(1).max(5).optional(), renovations:z.object({ kitchen:z.boolean().optional(), bath:z.boolean().optional(), roof:z.boolean().optional(), hvac:z.boolean().optional(), windows:z.boolean().optional() }).optional(), view:z.enum(["none","city","mountain","water"]).optional(), garageSpots:z.number().int().min(0).max(6).optional(), schoolRating:z.number().int().min(1).max(10).optional(), marketTrend:z.enum(["declining","flat","rising"]).optional(), comps:z.array(z.object({ price:z.number().positive(), sqft:z.number().positive() })).optional() });
export async function POST(req){
  try{
    const parsed=InputSchema.parse(await req.json());
    const geo=await geocodeAddress(parsed.address);
    let { sqft, lotSqft, yearBuilt }=parsed; const subject=await fetchSubjectFromEstated(geo.formatted).catch(()=>null);
  ...
  }catch(err){ return NextResponse.json({error:err?.message||"Unknown error"},{status:400}); }
}`,
