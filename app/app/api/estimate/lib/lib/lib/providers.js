...
export async function fetchSubjectFromEstated(address){
  ...
  const p=data?.data; if(!p) return null;
  return {
    sqft:p?.structure?.total_area||p?.building_size?.living_area,
    lotSqft:p?.lot?.lot_size||p?.lot_size?.lot_size_sq_ft,
    yearBuilt:p?.structure?.year_built||p?.year_built
  };
}

export async function fetchSchoolRating(lat,lng){
  const id=process.env.SCHOOLDIGGER_APP_ID, key=process.env.SCHOOLDIGGER_APP_KEY;
  if(!id||!key) return undefined;
  const url=new URL("https://api.schooldigger.com/v2.0/schools");
  url.searchParams.set("st","WA");
  url.searchParams.set("nearLatitude",String(lat));
  url.searchParams.set("nearLongitude",String(lng));
  url.searchParams.set("radiusMiles","3");
  ...
  const data=await res.json();
  const first=data?.schoolList?.[0];
  const rating=first?.rankHistory?.[0]?.rankScore;
  if(!rating) return undefined;
  let n=rating;
  if(rating<=5) n=(rating/5)*10; else if(rating<=100) n=(rating/100)*10;
  return Math.round(n);
}
...
export async function fetchCompsATTOM(lat,lng,sqft){
  ...
  if(sqft){
    url.searchParams.set("minbuildingareasqft",String(Math.floor(sqft*0.85)));
    url.searchParams.set("maxbuildingareasqft",String(Math.ceil(sqft*1.15)));
  }
  const res=await fetch(url,{headers:{apikey:key},cache:"no-store"}); if(!res.ok) return [];
  const data=await res.json();
  const sales=data?.sale||data?.sales||[];
  return sales.slice(0,12).map(s=>({
    price:+s.saleamt||0,
    sqft:+s.buildingareasqft||0,
    closingDate:s.salerecdate,
    distanceMiles:+s.distance||undefined
